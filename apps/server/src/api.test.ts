...
await test("API", async () => {
  ...
  await test("PATCH /users/:id", async () => {
    ...
    await it("updates the user", async () => {
      ...
      await fetch(
        endpoints.users + "/" + id,
        getFetchOptions("PATCH", { first: "Maxwell" })
      );
    ...
    await it("returns 400 if referenced role not found", async () => {
      ...
      const response = await fetch(
        endpoints.users + "/" + id,
        getFetchOptions("PATCH", { roleId: "not-a-role" })
      );
    ...
    await it("returns 404 if user not found", async () => {
      const response = await fetch(
        endpoints.users + "/not-a-user",
        getFetchOptions("PATCH", { first: "Bob" })
      );
  ...
  await test("PATCH /roles/:id", async () => {
    ...
    await it("updates the role", async () => {
      ...
      await fetch(
        endpoints.roles + "/" + id,
        getFetchOptions("PATCH", { name: "Security" })
      );
    ...
    await it("returns 404 if role not found", async () => {
      const response = await fetch(
        endpoints.roles + "/not-a-role",
        getFetchOptions("PATCH", { name: "Admin" })
      );
  ...
  await test("POST /roles", async () => {
    ...
    await it("returns a new role", async () => {
      ...
      assert.strictEqual(
        role.description,
        "Security engineers protect our products and services."
      );
    });
    ...
    await it("adds a new role", async () => {
      ...
      assert.strictEqual(
        role2.description,
        "Sales team members sell our products and services."
      );
    });
    ...
    await it("returns 400 if role name already exists", async () => {
      const response = await fetch(
        endpoints.roles,
        getFetchOptions("POST", { name: "Engineering" })
      );
  ...
  await test("DELETE /roles/:id", async () => {
    ...
    await it("updates all users with the role to the default role", async () => {
      ...
      const defaultRole = roles.data.find(
      ...
      const usersWithRole = usersBefore.data.filter(
        (user: { roleId: string }) => user.roleId === id
      );
